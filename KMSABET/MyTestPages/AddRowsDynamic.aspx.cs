...
namespace KMSABET
{
    public partial class AddRowsDynamic : System.Web.UI.Page
    {
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        RadioButtonList RBLGender1 = (RadioButtonList)grvStudentDetails.Rows[rowIndex].Cells[5].FindControl("RBLGender1");
                        DropDownList DrpQualification = (DropDownList)grvStudentDetails.Rows[rowIndex].Cells[6].FindControl("drpQualification");
                        ...
                        dtCurrentTable.Rows[i - 1]["Col5"] = RBLGender1.SelectedValue;
                        dtCurrentTable.Rows[i - 1]["Col6"] = DrpQualification.SelectedValue;
                        rowIndex++;
                    }
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        RBLGender1.SelectedValue = dt.Rows[i]["Col5"].ToString();
                        DrpQualification.SelectedValue = dt.Rows[i]["Col6"].ToString();
                        rowIndex++;
                    }
                }
            }
            ...
            try
            {
                ...
                if (table != null)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        string rbGen1 = row.ItemArray[5] as string;
                        string drpQual = row.ItemArray[6] as string;
                            ...
                            drpQual != null)
                        {
                            ...
                            Response.Write(string.Format("{0} {1} {2} {3} {4} {5}<br/>", txName, txAge, txAdd, rbGen, rbGen1, drpQual));
                        }
